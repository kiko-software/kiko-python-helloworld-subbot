...
def api_v1_messages_post():
    ...
    # is still contained in the first message. This is why the metadata is stored in a conversation-db.
    conversation = upsert_conversation_data(kiko_instance_identifier , conversation_id, metadata )
    ...
    if is_first_request == True:
        ...
        if parameter_question  is None:
            abort(400, description = "Missing question in the first parameter of the intent metadata")

        parameter_name = parameters[0]["name"]
        if parameter_name is None:
            abort(400, description = "Missing name in the first parameter of the intent metadata")
        
        # update session with the parameter name
        upsert_conversation_data(kiko_instance_identifier, conversation_id, last_input_parameter_identifier = parameter_name )
        ...
        # user input data are in "messages" - this should be the answer of the last question from the bot 
        ...
        if (len(messages) == 1 and messages[0]["type"] == 'message'):
            ...
            if(input_data["type"] == 'text/plain'):
                ...
                if input_parameter_identifier is None:
                  abort(400, description = "Missing input_parameter_identifier in the conversation")

                app.logger.info('input_parameter_identifier: %s', input_parameter_identifier )
...
def upsert_conversation_data(kiko_instance_identifier, conversation_id, metadata=None, last_input_parameter_identifier=None):
    """ Looks up (or creates) the session with the given conversation_session_id.
    """
    kiko_conversations = kiko_instances.document(kiko_instance_identifier).collection('kiko-conversations')
    ...
    if (last_input_parameter_identifier != None):
